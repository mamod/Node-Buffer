...
sub _likeNumber { looks_like_number($_[0]) }
...
sub new {
    ...
    if ( _isNumber($offset) ) {
        ...
        $self->{length} = _isNumber($encoding) && $encoding > 0 ? ceil($encoding) : 0;
        $self->{parent} = $subject->{parent} ? $subject->{parent} : $subject;
        $self->{offset} = $offset;
        
    } else {
        if ( _isNumber($subject) ) {
            $self->{length} = _isNumber($subject) && $subject > 0 ? ceil($subject) : 0;
        } elsif (!ref $subject){
            
        } elsif (ref $subject eq 'ARRAY'){
            $self->{length} = scalar @{$subject};
        } elsif ( _isBuffer($subject) ) {
            $self->{length} = _isNumber($subject->{length})
...
sub write {
    ...
    if ( _isString($offset) && !defined $length ) {
        $encoding = $offset;
        $offset = 0;
    #allow write(string, offset[, length], encoding)
    } elsif ( _likeNumber($offset)) {
        $offset = $offset+0;
        if ( _likeNumber($length) ) {
          $length = $length+0;
        } else {
          $encoding = $length;
          $length = undef;
...
sub toString {
    ...
    $encoding = lc ($encoding || 'utf8');
    if ( !_isNumber($start) || $start < 0) {
        $start = 0;
    } elsif ( $start > $self->{length} ) {
        $start = $self->{length};
    }
    
    if (!_isNumber($end) || $end > $self->{length} ) {
        $end = $self->{length};
    } elsif ($end < 0) {
        $end = 0;
    }
...
sub slice {
    ...
    my $buf = Node::Buffer->new($end - $start);
...
sub concat {
    ...
    my $buffer = Node::Buffer->new($length);
    my $pos = 0;
    foreach my $buf ( @{$list} ) {
        $buf->copy($buffer, $pos);
        $pos += $buf->length;
...
sub _clamp {
    ...
    return $defaultValue if (!_isNumber($index));
...
sub _isBuffer { return ref $_[0] eq 'Node::Buffer' }
sub isBuffer  { return ref $_[1] eq 'Node::Buffer' }
...
#=========================================================================
{
    ...
    my $writeDispatch = {
        ...
        ascii   => sub {
            #Node::Buffer::Encode::decode_utf8($_[0]);
